"""
Train model with Adaptive SVD (Phase 1: Proof-of-Concept)

This is a modified version of base_train.py that integrates adaptive SVD training
to detect and prevent catastrophic forgetting (clobbering) of learned patterns.

Phase 1 Configuration:
- Single layer: Only c_q in transformer block 0
- Small model: depth=4 recommended
- SVD analysis every 100 steps

Run as:
python base_train_adaptive_svd.py --depth=4 --max_seq_len=512 --device_batch_size=1 --eval_tokens=512 --core_metric_every=-1 --total_batch_size=512 --num_iterations=100

or distributed as:
torchrun --nproc_per_node=8 base_train_adaptive_svd.py
"""
...
# Adaptive SVD settings (Phase 1: Proof-of-Concept)
adaptive_svd = True # enable adaptive SVD training
adaptive_svd_target_layer = 0 # which layer to apply adaptive SVD to (Phase 1: single layer)
svd_interval = 20 # how often to run SVD analysis and potentially switch modes (steps)
...
model_config_kwargs = dict(
    ...
    adaptive_svd_target_layer=adaptive_svd_target_layer
)
...
def analyze_svd_layers(model, step):
    ...
    for block_idx, block in enumerate(orig_model.transformer.h):
        # Check c_q layer
        layer = block.attn.c_q
        full_name = f"block{block_idx}.attn.c_q"
        ...
        if layer.U_prev is None:
            ...
            print0(f"  [{full_name}] Initial SVD computed")
            continue
        ...
        if use_lowrank is True and layer.mode == "full":
            ...
            if layer.mode == "lowrank" and r_next != layer.r:
                ...
                layer.r = r_next
                print0(f"  [{full_name}] Updated r: {r_current} -> {r_next}")
...
for step in range(num_iterations + 1):
    ...
    # =========================================================================
    # ADAPTIVE SVD ANALYSIS (every svd_interval steps)
    # =========================================================================
    svd_decisions = {}
    ...
    if step % 100 == 0:
        ...
        # Add SVD metrics if available
        if svd_decisions:
            for layer_name, decision in svd_decisions.items():
                prefix = f"svd/{layer_name}/"
                log_dict[f"{prefix}mode"] = 1.0 if decision['mode'] == 'lowrank' else 0.0
                if decision.get('r') is not None:
                    log_dict[f"{prefix}r"] = decision['r']
                if 'principal_alignment' in decision:
                    log_dict[f"{prefix}principal_alignment"] = decision['principal_alignment']
                if 'minor_alignment' in decision:
                    log_dict[f"{prefix}minor_alignment"] = decision['minor_alignment']
                if 'subspace_angle' in decision:
                    log_dict[f"{prefix}subspace_angle"] = decision['subspace_angle']
                if 'reconstruction_error' in decision:
                    log_dict[f"{prefix}reconstruction_error"] = decision['reconstruction_error']
